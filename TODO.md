...
- **Initialize new bibnotes with workflow properties:** New bibnotes should include `status: to-read`, `note_type: bibnote`, `time_spent: 0` in frontmatter, plus the two buttons (`BUTTON[time-spent-increment]`, `BUTTON[resume-pdf]`) in the free text area. These should be configurable in the env. Potential collision with broader tags concept which might have "types" like Book, Paper, Thesis, Leaflet, Study Guide etc.
