...
- Forward-looking only -- concrete, startable work: tasks specified well enough that next-session-me can begin within ten minutes, plus a small number of deliberately parked capabilities with explicit triggers.
...
- Retain struck-through items through the next session because `TODO.md` is included in the standard filesdump while `HISTORY.md` normally is not; at the end of that next session, remove the already-archived items from `TODO.md`.
- Strategic direction, ordering, and milestones live in `GOALS.md`. Architecture, contracts, and settled decisions live in `README.md`.
- Keep this backlog lean. `pdf-annotations` exists to support work with books, highlights, and ideas in Obsidian; development beyond quick wins must justify the time it takes away from using that system.
