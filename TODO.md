...
- ~~**`streamline_annotations.py` Refactor:** Refactor `streamline_annotations.py` to fix duplicated logic and inefficiencies (currently postponed).~~ Landed 2026-07-24 as `AUDIT.md` A7. `process_objects` is the single state machine, `process_stream` a thin NDJSON wrapper, `streamline_annotations_list` a direct call. The deleted round-trip was also normalizing tuple to list, so the gate now accepts both -- read A7 before touching it.
