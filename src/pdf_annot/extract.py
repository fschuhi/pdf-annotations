...
def extract_highlight_text(page: fitz.Page, annot: fitz.Annot,
                           header_height: float, footer_height: float) -> str:
    """
    ...
    usable_rects = [
        r for r in rects
        if (r.y0 > header_height) and (r.y1 < page.rect.height - footer_height)
    ]
    if not usable_rects:
        return ""
        ...
        for annot in page.annots(types=TEXTUAL_ANNOTS):
            ...
            if annot.type[1].lower() == "highlight":
                info["extractedText"] = extract_highlight_text(
                    page, annot, header_height, footer_height
                )
...
def main():
    ...
    parser.add_argument(
        "--header-height", type=float, default=DEFAULT_HEADER_HEIGHT,
        help=f"Header height cutoff in points (default: {DEFAULT_HEADER_HEIGHT})."
    )
    parser.add_argument(
        "--footer-height", type=float, default=DEFAULT_FOOTER_HEIGHT,
    ...
    annotations = list(
        extract_annotations(doc, args.header_height, args.footer_height)
    )
